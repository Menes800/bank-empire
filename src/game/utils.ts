...
export function addEvent(state: GameState, nextEvent: GameEvent): GameState {
  return { ...state, events: [nextEvent, ...state.events].slice(0, 18) };
}
...
export function calculateRatios(
  ...
  const riskScore = clamp(
    12 +
      loanToDeposit * 24 +
      nplRatio * 4.2 +
      Math.max(0, 75 - compliance) * 0.5 +
      Math.max(0, 11 - capitalRatio) * 2.4,
    4,
    99,
  );
  const bankRunRisk = clamp(
    Math.max(0, 19 - liquidityRatio) * 2.5 +
      Math.max(0, 58 - compliance) * 0.35 +
      Math.max(0, 58 - reputation) * 0.72 +
      Math.max(0, 58 - satisfaction) * 0.42 +
      Math.max(0, 55 - (100 - riskScore)) * 0.25,
    0,
    100,
  );
