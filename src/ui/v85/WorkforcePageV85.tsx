...
function employeeStatus(employee: EmployeeProfile) {
  ...
  if (workload > 110 || wellbeing < 45) return { key: "risk", label: "At risk", reason: "Unsustainable role workload or low wellbeing" };
  if (workload > 92 || wellbeing < 62) return { key: "pressure", label: "Under pressure", reason: "This role is above the healthy workload range" };
  ...
  return { key: "steady", label: "Stable", reason: "Role workload and wellbeing are within the normal range" };
}
...
export function WorkforcePageV85({ game, action }: { game: GameState; action: GameAction }) {
  ...
  const managedBranch = selected ? game.branchOffices.find((branch) => branch.managerId === selected.id) : undefined;
  const selectedStatus = selected ? employeeStatus(selected) : undefined;

  const branchSummary = useMemo(() => game.branchOffices.map((branch) => {
    const employees = workforce.filter((employee) => employee.assignedBranchId === branch.id);
  ...
  return <>
    <section className="workforce-hero panel"><div><p className="eyebrow">WORKFORCE</p><h2>People, workplaces and role capacity</h2><p>Executives live on their own page. Branch managers hire locally within mandate, while this workspace is for development and deliberate transfers.</p></div><div className="workforce-health"><strong>{Math.round(averageWellbeing)}</strong><span>workforce wellbeing</span></div></section>
    ...
    <section className="panel branch-workforce-summary"><div className="panel-heading"><div><p className="eyebrow">LOCAL TEAMS</p><h3>Role pressure by branch</h3><p>Workload is calculated from actual lending, customers, growth activity, support work and management responsibility.</p></div></div><div className="branch-workforce-grid">{branchSummary.map(({ branch, employees, workload }) => <button key={branch.id} onClick={() => setFilter(branch.id)}><div><strong>{branch.name}</strong><small>{employees.length} assigned · {workload.toFixed(0)}% average role workload</small></div><span className={employees.length === 0 ? "negative" : workload > 95 ? "warning" : "positive"}>{employees.length === 0 ? "Unstaffed" : workload > 95 ? "Pressure" : "Staffed"}</span></button>)}</div></section>
