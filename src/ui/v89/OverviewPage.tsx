...
export function OverviewPageV89({ game, onOpenBoard, onOpenInbox }: { game: GameState; onOpenBoard: () => void; onOpenInbox: () => void }) {
  ...
  const news = game.events.filter((event) => !/ handled$|report ·|management report/i.test(event.title)).slice(0, 3);
  ...
  return <>
    ...
    <section className="v891-command-grid">
      <button className={ceoMatters.length ? "needs-action" : "healthy"} onClick={onOpenInbox}><span>CEO</span><div><small>AUTHORITY</small><strong>{ceoMatters.length ? `${ceoMatters.length} decision${ceoMatters.length === 1 ? "" : "s"} waiting` : "No decision waiting"}</strong><p>{ceoMatters.length ? "Open the exact escalations and blocking reasons." : "Executives are operating inside their mandates."}</p></div><b>Open →</b></button>
      <article className={branchAttention || creditExceptions || collections ? "watch" : "healthy"}><span>OPS</span><div><small>OPERATING PRESSURE</small><strong>{branchAttention + creditExceptions + collections} item{branchAttention + creditExceptions + collections === 1 ? "" : "s"} under pressure</strong><p>{branchAttention} branch · {creditExceptions} credit exception · {collections} collection</p></div></article>
      <article className={executiveVacancies || overloadedPeople ? "watch" : "healthy"}><span>MGMT</span><div><small>MANAGEMENT COVERAGE</small><strong>{executiveVacancies ? `${executiveVacancies} executive role${executiveVacancies === 1 ? "" : "s"} vacant` : "Executive team appointed"}</strong><p>{overloadedPeople} overloaded employee{overloadedPeople === 1 ? "" : "s"} · {activeProjects} active project{activeProjects === 1 ? "" : "s"}</p></div></article>
    </section>
    ...
    <section className="v89-overview-grid primary-grid v891-primary-grid">
      <article className="panel v89-performance-card">
        ...
        {historyReady ? <div className="v89-chart"><Sparkline points={game.history.slice(-45)} accessor={(point) => point.profit} /><div><span>Lifetime profit <b>{money.format(game.totalProfit)}</b></span><span>Credit losses <b>{money.format(game.creditLosses)}</b></span></div></div> : <div className="v891-onboarding"><strong>Run the bank forward to create a real trend.</strong><span>The first day creates cash movement. Thirty days triggers the first management review, branch accounting and executive actions.</span></div>}
      </article>

      <article className="panel v891-board-card">
        <div className="panel-heading"><div><p className="eyebrow">BOARD MANDATE</p><h3>{latestObjective?.title ?? "Quarterly objectives complete"}</h3><p>{latestObjective?.description ?? "The next board cycle will create new priorities."}</p></div><button className="text-button" onClick={onOpenBoard}>Boardroom →</button></div>
        <div className="v89-objective-list">{game.objectives.slice(0, 3).map((objective) => <div key={objective.id}><i className={cn(objective.completed && "done", objective.failed && "failed")} /><span><strong>{objective.title}</strong><small>{objective.completed ? "Completed" : objective.failed ? "Failed" : objective.description}</small></span></div>)}</div>
      </article>
    </section>

    {flow.days > 0 && <section className="panel v89-cash-card v891-cash-card">
      <div className="panel-heading"><div><p className="eyebrow">LIQUID CASH</p><h3>Cash movement · last {flow.days} days</h3><p>New lending, withdrawals, operations, funding and projects are separated below.</p></div><strong className={netCash >= 0 ? "positive" : "negative"}>{money.format(netCash)}</strong></div>
      <div className="v89-cash-lines"><CashItem label="Deposits in" value={flow.depositInflows} /><CashItem label="Loan repayments" value={flow.loanRepayments} /><CashItem label="Operating result" value={flow.operatingProfit} /><CashItem label="Funding change" value={flow.fundingChange} /><CashItem label="New lending" value={-flow.newLending} /><CashItem label="Withdrawals" value={-flow.customerWithdrawals} /><CashItem label="Projects & other" value={flow.otherMovements} /></div>
    </section>}

    <section className="panel v891-activity-panel">
      <div className="panel-heading"><div><p className="eyebrow">BANK ACTIVITY</p><h3>Management actions and material developments</h3><p>One chronological surface instead of separate report and news boxes.</p></div></div>
      {management.length || news.length ? <div className="v891-activity-grid">
        <div><small>MANAGEMENT</small>{management.length ? <div className="v89-feed">{management.map((item) => <div key={item.id}><span>{item.role}</span><div><strong>{item.title}</strong><small>{item.detail}</small></div>{item.amount !== undefined && <b>{money.format(item.amount)}</b>}</div>)}</div> : <p>No executive action recorded yet.</p>}</div>
        <div><small>GROUP NEWS</small>{news.length ? <div className="v89-feed">{news.map((item) => <div key={item.id}><i className={`news-dot ${item.tone}`} /><div><strong>{item.title}</strong><small>{item.body}</small></div><b>Day {item.day}</b></div>)}</div> : <p>No material development yet.</p>}</div>
      </div> : <div className="v891-onboarding"><strong>Your first operating cycle is ready.</strong><span>Advance time, review executive mandates and set branch priorities. This activity surface will then show what changed, who acted and what it cost.</span></div>}
    </section>
  </>;
}
