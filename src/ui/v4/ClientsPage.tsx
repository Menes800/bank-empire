...
export function ClientsPage({ game, action }: { game: GameState; action: GameAction }) {
  ...
  return <>
    <section className="credit-command-centre">
      <div><p className="eyebrow light">CUSTOMERS, CREDIT & COLLECTIONS</p><h2>Grow relationships without hiding the losses</h2><p>Price products, approve exceptional lending and manage missed payments through workout, collections and recovery.</p></div>
      <div className="credit-command-kpis"><span><small>Customers</small><strong>{game.customers.toLocaleString("en-GB")}</strong></span><span><small>Outstanding loans</small><strong>{money.format(outstanding)}</strong></span><span><small>Past-due relationships</small><strong>{problemLoans.length}</strong></span><span><small>Expected loss</small><strong>{money.format(expectedLoss)}</strong></span></div>
    </section>
    ...
    <section className="panel product-strategy-v7">
      <div className="panel-heading"><div><p className="eyebrow">PRODUCT PORTFOLIO</p><h3>Economics first, detailed terms second</h3><p>Each live product now shows customers, balance and estimated monthly contribution before you change its commercial preset.</p></div><span className="status good">{game.products.length} live</span></div>
    ...
    <section className="credit-operations-layout">
      ...
      <aside className="panel portfolio-control-panel"><div className="panel-heading"><div><p className="eyebrow">PORTFOLIO CONTROL</p><h3>Payment performance</h3></div><span className={problemLoans.length > 0 ? "status warn" : "status good"}>{problemLoans.length} problem loans</span></div>
