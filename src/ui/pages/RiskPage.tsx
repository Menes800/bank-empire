...
export function RiskPage({ game, action }: { game: GameState; action: GameAction }) {
  ...
  return <>
    ...
    <section className="panel v810-balance-sheet"><div className="panel-heading"><div><p className="eyebrow">BALANCE-SHEET DIAGNOSIS</p><h3>{fundingGap > 0 ? `Stable funding gap: ${money.format(fundingGap)}` : game.deposits < 100_000 ? "The loan book is funded, but almost entirely without customer deposits" : "The loan book is fully covered by stable funding"}</h3><p>Customer deposits, wholesale funding and positive equity all fund the loan book. A weak deposit mix still adds bounded risk, but it can no longer create impossible billion-point scores.</p></div></div><div className="v810-balance-flow"><span><small>Loan book</small><strong>{money.format(game.loans)}</strong></span><i>−</i><span><small>Customer deposits</small><strong>{money.format(game.deposits)}</strong></span><i>−</i><span><small>Wholesale funding</small><strong>{money.format(game.wholesaleFunding)}</strong></span><i>−</i><span><small>Positive equity</small><strong>{money.format(funding.equity)}</strong></span><i>=</i><span className={fundingGap > 0 ? "warning" : "good"}><small>Unfunded gap</small><strong>{money.format(fundingGap)}</strong></span></div></section>

    <section className="v810-treasury-grid">
      <article className="panel"><div><p className="eyebrow">WHOLESALE FUNDING</p><h3>{money.format(game.wholesaleFunding)}</h3><p>Current rate {game.wholesaleFundingRate.toFixed(2)}%. Borrowing raises cash immediately but adds daily interest expense and does not create customer deposits.</p></div><label className="v812-funding-size"><span>Transaction size</span><select value={fundingAmount} onChange={(event) => setFundingAmount(Number(event.target.value))}>{[5_000_000, 25_000_000, 50_000_000, 100_000_000].map((amount) => <option key={amount} value={amount}>{money.format(amount)}</option>)}</select></label><div className="v810-action-impact"><span><strong>Borrow {money.format(fundingAmount)}</strong><small>Cash and wholesale debt rise by the selected amount</small></span><button className="primary" onClick={() => action((state) => raiseWholesaleFunding(state, fundingAmount))}>Borrow</button></div><div className="v810-action-impact"><span><strong>Repay up to {money.format(fundingAmount)}</strong><small>Cash falls · wholesale debt and future interest expense fall</small></span><button className="secondary" disabled={game.wholesaleFunding <= 0 || game.cash <= 1_000_000} onClick={() => action((state) => repayWholesaleFunding(state, fundingAmount))}>Repay</button></div></article>
