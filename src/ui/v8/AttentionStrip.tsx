...
export function AttentionStrip({ game, onNavigate, compact = false }: { game: GameState; onNavigate: (page: string) => void; compact?: boolean }) {
  ...
  const openTasks = game.ceoInbox
    ...
    .sort((a, b) => (a.urgency === "critical" ? -1 : a.urgency === "important" ? 0 : 1) - (b.urgency === "critical" ? -1 : b.urgency === "important" ? 0 : 1));
  ...
  if (items.length < 3 && vacantBranch) items.push({ id: `manager-${vacantBranch.id}`, title: `Appoint a manager at ${vacantBranch.name}`, reason: "The branch cannot run automatically or improve performance without an accountable manager.", page: "network", action: "Open branches", tone: "warning" });
